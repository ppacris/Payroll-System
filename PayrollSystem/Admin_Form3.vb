Public Class Admin_Form4
    ...
    Private Sub RefreshGrid()
        Access.ExecQuery("SELECT * FROM Deleted_EmpData ORDER BY EmployeeID ASC")
        DataGridView1.DataSource = Access.DBDT
    End Sub
    Private Sub Admin_Form4_Load(sender As Object, e As EventArgs) Handles MyBase.Load

    End Sub

    Private Sub Admin_Form4_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        RefreshGrid()
    End Sub
    ...
    Private Sub btnRestore_Click(sender As Object, e As EventArgs) Handles btnRestore.Click
        ...
        ' RUN COMMAND
        Access.ExecQuery("INSERT INTO Employee_PayRoll ([EmployeeID],[Full_Name],[Tax],[SSS],[Philhealth],[PagIbig],[Total_Deduction],[Basic_Pay],[Gross_Pay],[Net_Pay]) " &
                         "VALUES ([EmployeeID],[Full_Name],[Tax],[SSS],[Philhealth],[PagIbig],[Total_Deduction],[Basic_Pay],[Gross_Pay],[Net_Pay]); ")

        ' CONFIRM DELETE
        If MsgBox("Are you sure that you want to Restore the selected user?", MsgBoxStyle.YesNo, "Confirm Restore") = MsgBoxResult.No Then Exit Sub

        ' DELETE SELECTED USER
        Access.AddParam("@EmployeeID", Label1.Text)
        Access.ExecQuery("DELETE FROM Deleted_EmpData WHERE EmployeeID=@EmployeeID")
